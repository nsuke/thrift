 ...
 * API values.
 */
static int toPthreadPolicy(PosixThreadFactory::Policy::value policy) {
  switch (policy) {
  case PosixThreadFactory::Policy::OTHER:
    return SCHED_OTHER;
  case PosixThreadFactory::Policy::FIFO:
    return SCHED_FIFO;
  case PosixThreadFactory::Policy::ROUND_ROBIN:
    return SCHED_RR;
  }
  return SCHED_OTHER;
}
 ...
 */
static int toPthreadPriority(PosixThreadFactory::Policy::value policy, PosixThreadFactory::Priority::value priority) {
...
#endif
  int quanta = (PosixThreadFactory::Priority::HIGHEST - PosixThreadFactory::Priority::LOWEST) + 1;
  float stepsperquanta = (float)(max_priority - min_priority) / quanta;

  if (priority <= PosixThreadFactory::Priority::HIGHEST) {
    return (int)(min_priority + stepsperquanta * priority);
  } else {
    // should never get here for priority increments.
    assert(false);
    return (int)(min_priority + stepsperquanta * PosixThreadFactory::Priority::NORMAL);
  }
}

PosixThreadFactory::PosixThreadFactory(Policy::value policy,
...
PosixThreadFactory::Priority::value PosixThreadFactory::getPriority() const {
  return priority_;
}

void PosixThreadFactory::setPriority(Priority::value value) {
  priority_ = value;
}
